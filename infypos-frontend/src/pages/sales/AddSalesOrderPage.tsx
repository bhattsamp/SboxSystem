...
import { salesOrderApi, warehouseApi, productApi, customerApi, voucherTypeApi } from '@/api'
...
export default function AddSalesOrderPage() {
  ...
  const [products, setProducts]         = useState<{ _id: string; name: string; sku?: string; salePrice?: number; taxRate?: number }[]>([])
  const [voucherTypes, setVoucherTypes] = useState<{ _id: string; name: string }[]>([])
  const [form, setForm] = useState({ warehouse: '', customer: '', voucherType: '', saleType: 'retail', shippingCost: 0, discountAmount: 0, note: '', terms: '' })
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const [whRes, custRes, prodRes, vtRes] = await Promise.all([
          warehouseApi.getAll({ limit: 100 }),
          customerApi.getAll({ limit: 100 }),
          productApi.getAll({ limit: 1000 }),
          voucherTypeApi.getAll({ limit: 100, module: 'sales' }),
        ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await salesOrderApi.create({
        ...
        note: form.note, terms: form.terms,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="page-header">
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate('/sales/orders')} className="btn-icon"><HiOutlineArrowLeft className="w-5 h-5" /></button>
          <div><h1 className="page-title">New Sales Order</h1><p className="page-subtitle">Create a sales order for a customer</p></div>
        </div>
        <Button type="submit" loading={loading}><HiOutlineSave className="w-4 h-4" />Save Sales Order</Button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <tbody>
                  {lines.map(({ item, total }) => (
                    <tr key={item.id}>
                      ...
                      <td><input type="number" min="0" step="0.01" value={item.unitPrice} onChange={e => updateItem(item.id, 'unitPrice', Number(e.target.value) || 0)} className="input input-sm w-24 font-bold" /></td>
        ...
        <div className="space-y-5">
          <div className="card space-y-4">
            ...
            <Select label="Sale Type" value={form.saleType} onChange={e => setForm({ ...form, saleType: e.target.value })}
              options={[{ value: 'retail', label: 'Retail' }, { value: 'cash', label: 'Cash' }]} />
