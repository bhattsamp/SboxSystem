...
export default function PurchaseOrderVoucherPage() {
  ...
  return (
    <VoucherLayout
      ...
      status={doc.status}
      from={{ label: 'Order To (Supplier)', name: doc.supplier?.name || '—', phone: doc.supplier?.phone, email: doc.supplier?.email, address: doc.supplier?.address }}
      to={{ label: 'Ship To', name: 'InfyPOS', address: 'Main Business Office' }}
      ...
      notes={doc.note}
      extraMeta={[
        { label: 'Order Status', value: doc.status || '—' },
      ]}
    />
  )
}
